...
import { X, Save, Bell, Shield, Clock, Loader2, BookmarkCheck, Code } from "lucide-react";
...
export default function EditMonitorModal({
  ...
  const [formData, setFormData] = useState({
    ...
    discord_webhook_url: monitor.alert_settings?.discord_webhook_url || "",
    notify_email: monitor.alert_settings?.notify_email || "",
    consecutive_failures_threshold:
      monitor.alert_settings?.consecutive_failures_threshold || 2,
  });

  const [headersJson, setHeadersJson] = useState(() => {
    if (!monitor.headers) return "";
    return typeof monitor.headers === "string"
      ? monitor.headers
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-xs">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-800 bg-slate-900 px-3 py-2 text-slate-200 focus:border-indigo-500 focus:outline-none"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <select
                ...
                className="rounded-md border border-slate-800 bg-slate-900 px-2.5 py-2 text-slate-200 focus:border-indigo-500 focus:outline-none"
              >
              ...
              <input
                ...
                className="flex-1 rounded-md border border-slate-800 bg-slate-900 px-3 py-2 text-slate-200 focus:border-indigo-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full rounded-md border border-slate-800 bg-slate-900 px-3 py-2 text-slate-200 placeholder-slate-700 focus:border-indigo-500 focus:outline-none"
            />
          </div>
          ...
          <div className="space-y-2 rounded-lg border border-slate-800 bg-slate-900/50 p-3">
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <input
                ...
                className="w-full rounded border border-slate-800 bg-slate-900 px-2.5 py-1.5 text-slate-200 placeholder-slate-700 focus:border-indigo-500 focus:outline-none"
              />
            </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-1.5 rounded-md bg-indigo-600 px-4 py-2 font-medium text-white hover:bg-indigo-500 transition disabled:opacity-50 cursor-pointer"
            >
              {loading ? (
